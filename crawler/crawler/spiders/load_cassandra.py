...
craigslist_schema = types.StructType([
    ...
    types.StructField('posting-id',types.StringType()),
...
def json_str(input_json):
    ...
    posting_id_str=str(input_json['posting-id'][0])
    beds_str=float(input_json['beds'][0]) 
    baths_str=float(input_json['baths'][0])
    ...
    input_json['posting-id']=posting_id_str
    input_json['beds']=beds_str
    input_json['baths']=baths_str
...
def main(inputs):  
    ...
    listings = listings_df.withColumnRenamed('posting-id','postingid')

    table="craigslistcanada3" #table in cluster
